...
export const Sidebar = () => {
    ...
    const handleSwitch = () => setOpen(prev => !prev)
                ...
                <ul className='flex-1'>
                    {menuItems.map(({ icons, label, path, action }) => (
                        <li
                            ...
                            onClick={action}
                        >
                            <Link href={path} className="px-2 py-2 my-3 bg-gray-800 transition-all duration-400 rounded-md cursor-pointer flex gap-5 items-center hover:bg-gradient-to-r from-[#f09819] via-[#ff5330] to-[#f09819]">
                                <div>{icons}</div>
